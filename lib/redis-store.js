...
export async function addSetMember(key, value, options = {}) {
  return redisCommand(['SADD', key, value], options);
}

export async function removeSetMember(key, value, options = {}) {
  return redisCommand(['SREM', key, value], options);
}

export async function getSetMembers(key, options = {}) {
  const values = await redisCommand(['SMEMBERS', key], options);
